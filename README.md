...
Uses SQLite as a lightweight local database (stock_data.db).

Dropdown (combobox) to select any available stock symbol/name from the database.

“Load Data” button to fetch and display rows for the selected stock.

Tabular display using ttk.Treeview (columns: Stock Name, Price, Timestamp).
